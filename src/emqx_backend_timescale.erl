...
pool_name(Pool) ->
  list_to_atom(lists:concat([emqx_backend_timescale,
    '_',
    Pool])).

register_metrics() ->
  [emqx_metrics:new(MetricName)
    || MetricName <- ['backend.timescale.message_publish']].

load() ->
  HookList =
    parse_hook(application:get_env(emqx_backend_timescale,
      hooks,
      [])),
  Templates = emqx_backend_timescale_cli:get_templates(),
  lists:foreach(fun ({Hook, Action, Pool, Filter}) ->
    load_(Hook,
      b2a(proplists:get_value(<<"function">>,
        Action)),
      {Filter, Pool, Templates})
                end,
    HookList),
...
load_(Hook, Fun, Params) ->
  case Hook of
    'message.publish' ->
      emqx:hook(Hook,
        fun emqx_backend_timescale:Fun/2,
        [Params])
  end.
...
unload_(Hook, Fun) ->
  case Hook of
    'message.publish' ->
      emqx:unhook(Hook, fun emqx_backend_timescale:Fun/2)
  end.
...
on_message_publish(Message = #message{topic = Topic},
    {Filter, Pool, Templates}) ->
  with_filter(fun () ->
    try emqx_backend_timescale_cli:prepare_data(Message,
      Templates)
    of
      Data ->
        emqx_metrics:inc('backend.timescale.message_publish'),
        emqx_backend_timescale_cli:pgsql_insert(Pool,
          Data),
        begin
          logger:log(debug,
            #{},
            ...
            fun (_) ->
              {logger_header()
                ++
                "Write point ~p to Timescale successfully",
                [Data]}
            end,
              mfa =>
              {emqx_backend_timescale,
